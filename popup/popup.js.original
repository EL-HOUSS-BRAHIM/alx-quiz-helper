...
document.addEventListener('DOMContentLoaded', function() {
    ...
    /**
     * Test function for inline onclick
     */
    window.testRefresh = function() {
        console.log('testRefresh: Inline onclick working!');
        alert('Refresh button clicked! Check console for details.');
        
        // Try to call the real handler if it exists
        if (typeof handleRefreshClick === 'function') {
            handleRefreshClick();
        } else {
            console.error('handleRefreshClick function not found');
        }
    };
